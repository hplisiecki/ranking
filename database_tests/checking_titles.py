...
new_links_path = r'D:\PycharmProjects\ranking\data\publications\orcid\export_to_automated'
...
for uni in unis:
    ...
    links = links[links['link'].str.contains('https', na = False)]
    print("Length old links: ", len(links))
    new_links = pd.read_csv(os.path.join(new_links_path, uni))
    print("Length new links: ", len(new_links))
    new_links = new_links[['name', 'link', 'title', 'id']]
    # merge on link and name
    try:
        links = links.merge(new_links, on=['name', 'link'], how='outer')
        print("Length missing titles: ", len(links[links['title'].isna()]))
    except:
