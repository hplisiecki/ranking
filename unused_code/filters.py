...
from PyPaperBot.__main__ import start





dois = links_failed.link.values
for doi in tqdm(dois):
    try:
        PyPaperBot.download(doi, 'swps')
    except:
        pass


def filter(links_failed):
    type = []
    direct_link = []
    for link in links_failed:
        try:
            ...
            elif ('journals.plos' in driver.current_url) and ('open access' not in soup.text):
            ...
            elif ('sci-hub' in driver.current_url) and ('Unfortunately' not in soup.text):
                pdf_link = driver.current_url
                type.append('pdf_site')
                direct_link.append(pdf_link)
                continue
            ...
            elif ('journals.plos' in driver.current_url) and ('open access' in soup.text):
                type.append('text_scraping')
                direct_link.append(driver.current_url)
            ...
            if 'sciendo' in driver.current_url and 'Page not found' in soup.text:
                type.append('manual_checking')
                direct_link.append(link)
                continue
...
type_list, direct_link = filter(links_failed.links.values)
links_failed['type'] = type_list
...
types, directs = filter(unavailable)
