...
wos = pd.read_csv('data/swps_wos_links.csv')
# drop nans from links
wos = wos.dropna(subset=['doi'])
wos['name'] = wos['fullname']
wos['link'] = ['https://doi.org/' + l for l in wos['doi']]
del wos['fullname']
del wos['doi']

# merge
publications = pd.concat([publications, wos], ignore_index=True)
publications = publications.drop_duplicates(subset=['link'])
# reset index
publications = publications.reset_index(drop=True)
# save
publications.to_csv('data/swps_publication_merged.csv', index=False)
##################################################################################


# adding information about titles and ids ########################################
publications = pd.read_csv('data/swps_publications_merged.csv')
ids = []
...
publications.to_csv('data/swps_publications_merged_ids.csv', index=False)
##################################################################################

# load
# automatic filling of the links, not used for some reason #######################
publications = pd.read_csv('data/swps_publications_merged_ids.csv')

titles = publications[publications.is_a_link == 0].link.values

titles_df = publications[publications.is_a_link == 0]
...
for idx, link in enumerate(links):
    if link != 'https://sci-hub.se/':
        ...
        # change the value in the dataframe
        publications.iloc[index, 2] = link
...
# save
titles_df.to_csv('data/swps_titles.csv', index=False)

# load
titles_df = pd.read_csv('data/swps_titles.csv')

# Downloading the pdfs ###########################################################
from scihub_api import SciHub
import pandas as pd
from tqdm import tqdm
save_path = r'D:\data\ranking\swps/'

# load
publications = pd.read_csv('data/swps_publications_merged_ids.csv')

publications_with_links = publications[publications.is_a_link == 1].reset_index(drop=True)
sh = SciHub()
failed = []
...
# save
publications_with_links.to_csv('data/swps_publications_with_links_after_auto_pdf.csv', index=False)
##################################################################################
...
#     try:
#         path = f'https://sci-hub.se/{link}'
#         driver.get(path)
#         # download pdf
#         time.sleep(2)
#         html = driver.page_source
#         soup = bs(html, 'html.parser')
#         pdf = soup.find('embed', {'id': 'pdf'})
#         pdf_link = 'https:' + pdf['src']
#         # save pdf
#         pdf = requests.get(pdf_link)
#         with open(save_path + f'{idx}.pdf', 'wb') as f:
#             f.write(pdf.content)
#     except:
#         failed.append(idx)
